...
public class GoogleTravelTimeService : ITravelTimeService
{
        ...
        CancellationToken ct = default)
    {
        string travelMode = mode switch
        {
            ...
            TransportMode.Bus => "transit", // Transit hỗ trợ cực mạnh ở Google Maps
            _ => "driving"
        };
        ...
        for (int i = 0; i < origins.Count; i++)
        {
            ...
            for (int j = 0; j < destinations.Count; j++)
            {
                ...
                {
                    matrix[i, j] = element.Duration.Value; // Tính bằng giây (Seconds)
                }
                else
                {
                    matrix[i, j] = 999999; // Penalty Rất lớn nếu Route not found
                }
            }
        }
...
internal class GoogleMatrixElement
{
    ...
    public GoogleMatrixDuration? Duration { get; set; }
}

internal class GoogleMatrixDuration
{
